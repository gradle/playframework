...
To use the Play plugin, include the following in your build script to apply the `com.playframework.play` plugin. Currently, the plugin is only available on the Gradle plugin portal.
...
.build.gradle
----
include::{samplesCodeDir}/apply-plugin/groovy/build.gradle[]
----